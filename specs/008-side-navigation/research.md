...
**Decision**: `position: fixed` on the right edge, `top: 50%` with a `translateY(-50%)` to centre it vertically.
...
- *`position: sticky` inside a flex sidebar* — keeps the menu in normal flow, which is tidier conceptually, but it requires the page to become a two-column layout. Every existing page assumes one centred column, so this would touch far more than the navigation. Rejected as disproportionate.
...
**Decision**: `aria-current="page"` written at build time by comparing `page.url`, with an exact match for single pages and a `contains` match for sections that have child pages. Styled with both a colour change and a right border.

**Rationale**: `aria-current` is the attribute the platform provides for exactly this, so screen readers announce it without any extra authoring. Doing the comparison in Liquid means it is correct in the delivered HTML rather than applied after load. Stories and bookmarks have child pages — an individual story, a filter page — and a visitor reading one is still in that section, so those match by prefix while `/research/` and `/contact/` match exactly. The border matters: colour alone would fail the same accessibility bar the journey categories are held to.

**Alternatives considered**:

- *Colour alone for the current link* — simplest, but conveys the state only to visitors who can distinguish the two colours. Rejected for the same reason FR-013 exists in feature 005.
